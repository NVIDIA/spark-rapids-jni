...
TEST_F(UtilitiesTest, BitwiseOr)
{
  ...
  // 2 buffers
  {
    ...
    cudaMemcpyAsync(da.data(), a.data(), sizeof(cudf::bitmask_type) * a.size(), cudaMemcpyHostToDevice);
    ...
    cudaMemcpyAsync(db.data(), b.data(), sizeof(cudf::bitmask_type) * b.size(), cudaMemcpyHostToDevice);

    std::vector<cudf::bitmask_type> expect{0x11111001, 0xffffffff, 0xffff0000, 0xf101010f, 0xab0100ab};
    auto d_result = spark_rapids_jni::bitmask_bitwise_or({{&da, &db}}, stream);
    CUDF_EXPECTS(d_result.size() == expect.size(), "Unexpected output size");
    std::vector<cudf::bitmask_type> result(d_result.size());
    cudaMemcpy(result.data(), d_result.data(), sizeof(cudf::bitmask_type) * d_result.size(), cudaMemcpyDeviceToHost);
    CUDF_EXPECTS(std::equal(result.begin(), result.end(), expect.begin()), "Unexpected output size");
  }
  ...
  // 4 buffers
  {
    ...
    cudaMemcpyAsync(dc.data(), c.data(), sizeof(cudf::bitmask_type) * c.size(), cudaMemcpyHostToDevice);
    ...
    cudaMemcpyAsync(dd.data(), d.data(), sizeof(cudf::bitmask_type) * d.size(), cudaMemcpyHostToDevice);

    std::vector<cudf::bitmask_type> expect{0x10011001, 0x0000ffff, 0xffff0000, 0x01010101, 0xab0000ab};
    auto d_result = spark_rapids_jni::bitmask_bitwise_or({{&da, &db, &dc, &dd}}, stream);
    CUDF_EXPECTS(d_result.size() == expect.size(), "Unexpected output size");
    std::vector<cudf::bitmask_type> result(d_result.size());
    cudaMemcpy(result.data(), d_result.data(), sizeof(cudf::bitmask_type) * d_result.size(), cudaMemcpyDeviceToHost);
    CUDF_EXPECTS(std::equal(result.begin(), result.end(), expect.begin()), "Results do not match expected");
  }
}
