...
std::vector<bool> extract_validity_mask(cudf::column_view const& col)
{
  ...
  if (col.null_mask() != nullptr) {
    ...
    CUDF_CUDA_TRY(
      cudaMemcpy(mask_host.data(), col.null_mask(), mask_size_bytes, cudaMemcpyDeviceToHost));
...
TEST_F(MixedSortMergeJoinTest, LeftJoinEmptyRight)
{
  ...
  // Expected: all left rows with null right indices, which are marked as out-of-bounds =
  // right_num_rows (0)
...
TEST_F(MixedSortMergeJoinTest, LeftJoinGatherNullify)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(expected, gathered_right_tbl->view().column(0));
}
...
TEST_F(MixedSortMergeJoinTest, PreSortedTables)
{
  ...
  auto [left_result, right_result] =
    spark_rapids_jni::mixed_sort_merge_inner_join(left_eq_table,
                                                  right_eq_table,
                                                  left_cond_table,
                                                  right_cond_table,
                                                  condition,
...
TEST_F(MixedSortMergeJoinTest, InnerJoinNullsInConditionalColumns)
{
  ...
  cudf::test::fixed_width_column_wrapper<int32_t> right_equality({1, 2, 3});
  cudf::test::fixed_width_column_wrapper<int32_t> left_conditional({10, 20, 30},
                                                                   {true, false, true});
  cudf::test::fixed_width_column_wrapper<int32_t> right_conditional({5, 15, 25},
                                                                    {true, true, false});
...
TEST_F(MixedSortMergeJoinTest, LeftJoinWithMultipleDataColumns)
{
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (left_row == 1) {
      ...
      if (right_data1_valid[i]) { EXPECT_EQ(1000, right_data1_host[i]); }
      if (right_data2_valid[i]) { EXPECT_EQ(50, right_data2_host[i]); }
    } else {
      // Rows 0 and 2 don't match: should be null
      EXPECT_FALSE(right_data1_valid[i])
        << "Right data1 should be null for unmatched row " << left_row;
      EXPECT_FALSE(right_data2_valid[i])
        << "Right data2 should be null for unmatched row " << left_row;
    }
  }
}
...
TEST_F(MixedSortMergeJoinTest, LargeRandomDataSemiJoin)
{
  ...
  cudf::test::fixed_width_column_wrapper<int32_t> left_equality(left_eq_data.begin(),
                                                                left_eq_data.end());
  cudf::test::fixed_width_column_wrapper<int32_t> right_equality(right_eq_data.begin(),
                                                                 right_eq_data.end());
  cudf::test::fixed_width_column_wrapper<int32_t> left_conditional(left_cond_data.begin(),
                                                                   left_cond_data.end());
  cudf::test::fixed_width_column_wrapper<int32_t> right_conditional(right_cond_data.begin(),
                                                                    right_cond_data.end());
  ...
  EXPECT_EQ(unique_indices.size(), result_host.size())
    << "Semi-join result should have no duplicates";
}
...
TEST_F(MixedSortMergeJoinTest, EqualityOnlyLeftJoinEmptyRight)
{
  ...
  cudf::test::fixed_width_column_wrapper<int32_t> expected_right(
    {0, 0, 0});  // All OOB (right_num_rows=0)
...
{
  if (data.empty()) { return cudf::make_empty_column(cudf::type_id::STRING); }
  ...
  CUDF_CUDA_TRY(cudaMemcpyAsync(
    d_chars.data(), chars.data(), chars.size(), cudaMemcpyHostToDevice, stream.value()));
...
TEST_F(MixedSortMergeJoinTest, LargeScaleConditionalLeftJoin)
{
  ...
  cudf::test::fixed_width_column_wrapper<int32_t> right_hour(right_hour_data.begin(),
                                                             right_hour_data.end());
  cudf::test::fixed_width_column_wrapper<int64_t> right_start_time(right_start_time_data.begin(),
                                                                   right_start_time_data.end());
  cudf::test::fixed_width_column_wrapper<int64_t> right_end_time(right_end_time_data.begin(),
                                                                 right_end_time_data.end());
  ...
  auto ge_expr =
    cudf::ast::operation(cudf::ast::ast_operator::GREATER_EQUAL, left_time_ref, right_start_ref);
  auto le_expr =
    cudf::ast::operation(cudf::ast::ast_operator::LESS_EQUAL, left_time_ref, right_end_ref);
