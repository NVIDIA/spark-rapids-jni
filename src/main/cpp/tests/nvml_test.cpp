...
class NVMLTest : public ::testing::Test {
 protected:
  ...
  {
    if (nvml_result != NVML_SUCCESS || !nvml.device_get_count || !nvml.device_get_handle_by_index) {
      return nullptr;
    }
...
TEST_F(NVMLTest, NVMLDeviceGetName_Succeeds)
{
  ...
  nvmlReturn_t result = nvml.device_get_name(device, name, NVML_DEVICE_NAME_BUFFER_SIZE);
...
TEST_F(NVMLTest, NVMLDeviceGetBrand_Succeeds)
{
  ...
  nvmlReturn_t result = nvml.device_get_brand(device, &brandType);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetUtilizationRates_Succeeds)
{
  ...
  nvmlReturn_t result = nvml.device_get_utilization_rates(device, &utilization);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetMemoryInfo_Succeeds)
{
  ...
  nvmlMemory_t memory;
  nvmlReturn_t result = nvml.device_get_memory_info(device, &memory);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetTemperature_Succeeds)
{
  ...
  unsigned int temp;
  nvmlReturn_t result = nvml.device_get_temperature(device, NVML_TEMPERATURE_GPU, &temp);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetPowerUsage_Succeeds)
{
  ...
  unsigned int power;
  nvmlReturn_t result = nvml.device_get_power_usage(device, &power);
...
TEST_F(NVMLTest, NVMLDeviceGetPowerManagementLimit_Succeeds)
{
  ...
  unsigned int limit;
  nvmlReturn_t result = nvml.device_get_power_management_limit(device, &limit);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetClockInfo_Graphics_Succeeds)
{
  ...
  unsigned int clock;
  nvmlReturn_t result = nvml.device_get_clock_info(device, NVML_CLOCK_GRAPHICS, &clock);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetClockInfo_Memory_Succeeds)
{
  ...
  unsigned int clock;
  nvmlReturn_t result = nvml.device_get_clock_info(device, NVML_CLOCK_MEM, &clock);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetClockInfo_SM_Succeeds)
{
  ...
  unsigned int clock;
  nvmlReturn_t result = nvml.device_get_clock_info(device, NVML_CLOCK_SM, &clock);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetNumGpuCores_Succeeds)
{
  ...
  nvmlReturn_t result = nvml.device_get_num_gpu_cores(device, &num_cores);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetPerformanceState_Succeeds)
{
  ...
  nvmlReturn_t result = nvml.device_get_performance_state(device, &p_state);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetFanSpeed_Succeeds)
{
  ...
  unsigned int speed;
  nvmlReturn_t result = nvml.device_get_fan_speed(device, &speed);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetCurrPcieLinkGeneration_Succeeds)
{
  ...
  unsigned int gen;
  nvmlReturn_t result = nvml.device_get_curr_pcie_link_generation(device, &gen);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetCurrPcieLinkWidth_Succeeds)
{
  ...
  unsigned int width;
  nvmlReturn_t result = nvml.device_get_curr_pcie_link_width(device, &width);
  EXPECT_TRUE(result == NVML_SUCCESS || result == NVML_ERROR_NOT_SUPPORTED);
}
...
TEST_F(NVMLTest, NVMLDeviceGetTotalEccErrors_SingleBit_Succeeds)
{
  ...
  nvmlReturn_t result =
    nvml.device_get_total_ecc_errors(device, NVML_SINGLE_BIT_ECC, NVML_VOLATILE_ECC, &ecc_count);
