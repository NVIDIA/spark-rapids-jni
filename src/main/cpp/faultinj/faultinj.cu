...
CUptiResult cuptiInitialize(void) {
  ...
  CUPTI_CALL(cuptiSubscribe(&globalControl.subscriber,
                            (CUpti_CallbackFunc)faultInjectionCallbackHandler,
                            nullptr));

  // TODO do this dynamically based on config?
  CUPTI_CALL(cuptiEnableDomain(1, globalControl.subscriber,
...
lookupConfig(boost::optional<boost::property_tree::ptree &> domainConfigs,
             const char *key, CUpti_CallbackId cbid) {
...
void CUPTIAPI faultInjectionCallbackHandler(void *, CUpti_CallbackDomain domain,
                                            CUpti_CallbackId cbid,
                                            void *cbdata) {
  const std::string faultInjectorKernelPrefix =
      std::string("faultInjectorKernel");
  ...
  boost::optional<boost::property_tree::ptree &> matchedFaultConfig =
      boost::none;
  ...
  if (domain == CUPTI_CB_DOMAIN_DRIVER_API &&
      globalControl.driverFaultConfigs) {
    switch (cbid) {
    ...
    case CUPTI_DRIVER_TRACE_CBID_cuLaunchHostFunc_ptsz:
      ...
      if (std::string(cbInfo->symbolName)
              .compare(0, faultInjectorKernelPrefix.size(),
                       faultInjectorKernelPrefix) == 0) {
    ...
    default:
      matchedFaultConfig = lookupConfig(globalControl.driverFaultConfigs,
                                        cbInfo->functionName, cbid);
    }
  }
  if (domain == CUPTI_CB_DOMAIN_RUNTIME_API &&
    ...
    case CUPTI_RUNTIME_TRACE_CBID_cudaGraphLaunch_ptsz_v10000:
      ...
      // intentional fallthrough
    default:
      matchedFaultConfig = lookupConfig(globalControl.runtimeFaultConfigs,
                                        cbInfo->functionName, cbid);
    }
  }
  ...
  const int substituteReturnCode =
      (*matchedFaultConfig)
          .get_optional<int>(substituteReturnCodeKey)
          .value_or(static_cast<int>(CUDA_SUCCESS));

  const int injectionProbability =
      (*matchedFaultConfig).get_optional<int>(percentKey).value_or(0);
  ...
  if (interceptionCount <= 0) {
    spdlog::trace("skipping interception because hit count reached 0, "
                  ...
                  "interceptionCount={}",
                  domain, cbInfo->functionName, injectionType,
                  injectionProbability, interceptionCount);
    return;
  }
  ...
  if (injectionProbability < 100) {
    ...
    spdlog::debug("matched config based on rand10000={} skipThreshold={} "
                  "domain={} function={} injectionType={} probability={}",
                  rand10000, skipThreshold, domain, cbInfo->functionName,
                  injectionType, injectionProbability);
  } else {
    spdlog::debug("matched 100% config domain={} function={} injectionType={} "
                  "probability={}",
                  domain, cbInfo->functionName, injectionType,
  ...
  if (interceptionCount != INT_MAX) {
    spdlog::debug("updating interception count {}: before locking",
                  interceptionCount);
    ...
    const int interceptionCount =
        (*matchedFaultConfig).get<int>("interceptionCount");
  ...
  case FI_RETURN_VALUE:
    if (domain == CUPTI_CB_DOMAIN_DRIVER_API) {
      ...
      *cuResPtr = static_cast<CUresult>(substituteReturnCode);
      ...
      cudaError_t *cudaErrPtr =
          static_cast<cudaError_t *>(cbInfo->functionReturnValue);
      spdlog::error(
          "updating runtime return value DOES NOT WORK, use trap or assert");
...
void readFaultInjectorConfig(void) {
  ...
  try {
    ...
    const int logLevel =
        globalControl.configRoot.get_optional<int>(logLevelKey).value_or(0);

    globalControl.dynamic =
        globalControl.configRoot.get_optional<bool>(dynamicConfigKey)
            .value_or(false);

    const spdlog::level::level_enum logLevelEnum =
    ...
    globalControl.driverFaultConfigs =
        globalControl.configRoot.get_child_optional(driverFaultsKey);
    globalControl.runtimeFaultConfigs =
    ...
    spdlog::error("error parsing fault injector config, still editing? {}",
                  error.what());
  }
  PTHREAD_CALL(pthread_rwlock_unlock(&globalControl.configLock));
  jsonStream.close();
  spdlog::debug("readFaultInjectorConfig from {} DONE",
...
void traceConfig(boost::property_tree::ptree const &pTree) {
  ...
  for (boost::property_tree::ptree::const_iterator it = pTree.begin();
       it != end; ++it) {
    spdlog::trace("congig key={} value={}", it->first,
...
void *dynamicReconfig(void *) {
  ...
  const int watchFd = inotify_add_watch(
      inotifyFd, globalControl.configFilePath.c_str(), IN_MODIFY);
  if (watchFd < 0) {
  ...
  if (watchFd >= 0) {
    spdlog::debug("config watcher thread: inotify_rm_watch {} {}", inotifyFd,
                  watchFd);
...
extern "C" int InitializeInjection(void) {
  ...
  if (globalControl.dynamic) {
    spdlog::debug(
        "creating a thread to watch the fault injector config interactively");
    PTHREAD_CALL(pthread_create(&globalControl.dynamicThread, nullptr,
                                dynamicReconfig, nullptr));
  }
