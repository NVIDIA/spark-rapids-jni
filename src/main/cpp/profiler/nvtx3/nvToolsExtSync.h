 ...
 */
#define NVTX_SYNCUSER_ATTRIB_STRUCT_SIZE \
  (NVTX_STATIC_CAST(uint16_t, sizeof(nvtxSyncUserAttributes_v0)))
/** \endcond */
 ...
 *          bool acquired = __sync_bool_compare_and_swap(&bLocked, 0, 1); // atomic compiler
 * intrinsic
 *
 *          if (acquired) {
 ...
 * a user defined synchronization object.  The tools will return a pointer through the API for the
 * application to hold on its behalf to reference the string in the future.
 *
 */
