/*
 * SPDX-FileCopyrightText: Copyright (c) 2020-2025 NVIDIA CORPORATION & AFFILIATES. All rights
...
NVTX3_INLINE_IF_REQUESTED namespace NVTX3_VERSION_NAMESPACE
{
  ...
  class named_category_in final : public category {
   public:
     ...
     */
    template <typename C,
              typename std::enable_if<!detail::has_name<C>::value || !detail::has_id<C>::value,
                                      int>::type = 0>
    NVTX3_NO_DISCARD static named_category_in const& get() noexcept
    {
      NVTX3_STATIC_ASSERT(
  ...
  class registered_string_in {
   public:
    ...
    template <
      typename M,
      typename std::enable_if<detail::is_c_string<decltype(M::message)>::value, int>::type = 0>
  ...
  class event_attributes {
   public:
    ...
    NVTX3_CONSTEXPR_IF_CPP14 explicit event_attributes(category const& c,
     ...
     */
    template <typename... Args>
    NVTX3_CONSTEXPR_IF_CPP14 explicit event_attributes(payload const& p,
                                                       Args const&... args) noexcept
      : event_attributes(args...)
    {
     ...
     */
    template <typename... Args>
    NVTX3_CONSTEXPR_IF_CPP14 explicit event_attributes(message const& m,
                                                       Args const&... args) noexcept
      : event_attributes(args...)
    {
