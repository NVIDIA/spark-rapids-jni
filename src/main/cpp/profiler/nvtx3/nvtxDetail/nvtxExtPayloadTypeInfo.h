...
#define MKTYPEDEF(type) \
  typedef struct {      \
    char c;             \
    type d;             \
  } _nvtx_##type
#define MKTYPEDEF2(type, tname) \
  typedef struct {              \
    char c;                     \
    type d;                     \
  } _nvtx_##tname
...
/* `char8_t` is available as of C++20 or C23 */
     ...
     (defined(__cplusplus) && __cplusplus >= 201811L)) &&          \
  !defined(__APPLE__)
MKTYPEDEF(char8_t);
...
#if ((defined(__STDC_VERSION__) && __STDC_VERSION__ >= 201112L) || \
     (defined(__cplusplus) && __cplusplus >= 200704L)) &&          \
  !defined(__APPLE__)
MKTYPEDEF(char16_t);
MKTYPEDEF(char32_t);
...
extern
#endif
  const nvtxPayloadEntryTypeInfo_t NVTX_EXT_PAYLOAD_VERSIONED_ID(
    nvtxExtPayloadTypeInfo)[NVTX_PAYLOAD_ENTRY_TYPE_INFO_ARRAY_SIZE] = {
    ...
    /* NVTX_PAYLOAD_ENTRY_TYPE_UCHAR */
    {sizeof(unsigned char), nvtx_alignof2(unsigned char, uchar)},
    /* NVTX_PAYLOAD_ENTRY_TYPE_SHORT */ {sizeof(short), nvtx_alignof(short)},
    /* NVTX_PAYLOAD_ENTRY_TYPE_USHORT */
    {sizeof(unsigned short), nvtx_alignof2(unsigned short, ushort)},
    ...
    /* NVTX_PAYLOAD_ENTRY_TYPE_ULONG */
    {sizeof(unsigned long), nvtx_alignof2(unsigned long, ulong)},
    /* NVTX_PAYLOAD_ENTRY_TYPE_LONGLONG */ {sizeof(long long), nvtx_alignof2(long long, longlong)},
    /* NVTX_PAYLOAD_ENTRY_TYPE_ULONGLONG */
    {sizeof(unsigned long long), nvtx_alignof2(unsigned long long, ulonglong)},
    ...
    /* NVTX_PAYLOAD_ENTRY_TYPE_LONGDOUBLE */
    {sizeof(long double), nvtx_alignof2(long double, longdouble)},

    /* NVTX_PAYLOAD_ENTRY_TYPE_SIZE */ {sizeof(size_t), nvtx_alignof(size_t)},
    /* NVTX_PAYLOAD_ENTRY_TYPE_ADDRESS */
    {sizeof(nvtx_payload_pointer_type), nvtx_alignof(nvtx_payload_pointer_type)},
