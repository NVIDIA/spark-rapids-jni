...
nvtxMemHeapReset(nvtxDomainHandle_t domain,
 ...
 * virtual memory to reference the region in other related functions which accept
 * nvtxMemRegionRef_t.
 */
typedef struct nvtxMemRegionsRegisterBatch_v1 {
  ...
  nvtxMemRegionHandle_t*
    regionHandleElementsOut;      /* This will also become the handle for this region. */
 ...
 */
NVTX_DECLSPEC void NVTX_API nvtxMemRegionsRegister(nvtxDomainHandle_t domain,
...
/** \brief Register a region of memory inside of a heap of linear process virtual memory
 */
NVTX_DECLSPEC void NVTX_API nvtxMemRegionsResize(nvtxDomainHandle_t domain,
...
NVTX_DECLSPEC void NVTX_API nvtxMemRegionsName(nvtxDomainHandle_t domain,
...
NVTX_DECLSPEC void NVTX_API nvtxMemPermissionsAssign(nvtxDomainHandle_t domain,
...
NVTX_DECLSPEC nvtxMemPermissionsHandle_t NVTX_API nvtxMemPermissionsCreate(
  nvtxDomainHandle_t domain, int32_t creationflags); /* NVTX_MEM_PERMISSIONS_CREATE_FLAGS_* */
...
NVTX_DECLSPEC void NVTX_API nvtxMemPermissionsDestroy(
  ...
  nvtxMemPermissionsHandle_t
    permissionsHandle); /* only supported on objects from nvtxMemPermissionsCreate */

/** \brief Reset the permissions object back to its created state. */
NVTX_DECLSPEC void NVTX_API nvtxMemPermissionsReset(nvtxDomainHandle_t domain,
...
/** \brief Upon binding, with the thread, exclude parent scope write regions instead of overlaying
 *on top of them.
 *
...
/** \brief Upon binding, with the thread, exclude parent scope read regions instead of overlaying on
 *top of them.
 *
...
/** \brief Upon binding, with the thread, exclude parent scope atomic RW regions instead of
 *overlaying on top of them.
 *
 ...
 * execution is honoring the permissions as well as the state of
 *NVTX_MEM_PERMISSIONS_HANDLE_PROCESS_WIDE at the time of binding.  If this is not bound then
 *NVTX_MEM_PERMISSIONS_HANDLE_PROCESS_WIDE should be used to validate the memory.
 *
...
NVTX_DECLSPEC void NVTX_API nvtxMemPermissionsBind(
  nvtxDomainHandle_t domain,
  nvtxMemPermissionsHandle_t
    permissions, /* special object like NVTX_MEM_PERMISSIONS_HANDLE_PROCESS_WIDE are not supported
                  */
