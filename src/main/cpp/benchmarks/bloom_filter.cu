...
static void bloom_filter_put(nvbench::state& state)
{
  ...
  state.exec(nvbench::exec_tag::timer | nvbench::exec_tag::sync,
    [&](nvbench::launch& launch, auto& timer) {
      timer.start();
      spark_rapids_jni::bloom_filter_put({reinterpret_cast<cudf::bitmask_type*>(bloom_filter->data()), bloom_filter->size()}, bloom_filter_bits, *input, num_hashes);
      stream.synchronize();
      timer.stop();
...
NVBENCH_BENCH(bloom_filter_put)
  ...
  .add_int64_axis("bloom_filter_bytes", {512 * 1024, 1024 * 1024, 2 * 1024 * 1024, 4 * 1024 * 1024, 8 * 1024 * 1024});
