    ...
    if (method == 0) {
      ...
      for (int i = 0; i < 5; ++i) {
        if (paths[i].size() > 0) {
          ...
          constexpr int path_size = 32;

          std::size_t offset = 0;
          std::vector<std::vector<std::tuple<path_instruction_type, std::string, int64_t>>> segment;
          segment.reserve(path_size);
          while (offset < paths[i].size()) {
            auto const last = std::min(offset + path_size, paths[i].size());
            segment.resize(0);
            segment.insert(segment.end(), paths[i].begin() + offset, paths[i].begin() + last);
            offset += path_size;

            [[maybe_unused]] auto const output = spark_rapids_jni::get_json_object_multiple_paths(
              cudf::strings_column_view{chunk.tbl->get_column(i).view()}, segment);
          }
        }
      }
    }
...
#if 0
void print_nodes(std::vector<spark_rapids_jni::path_node> const& nodes)
{
  std::cout << "\n\n\nHost data\n";
  int count{0};
  for (auto& node : nodes) {
    ...
    std::cout << " "
              << "type: " << static_cast<int>(node.data.type) << std::endl;
    std::cout << " "
              << "name: " << node.data.name << std::endl;
    std::cout << " "
              << "index: " << node.data.index << std::endl;
    std::cout << " "
              << "parent: " << node.parent << std::endl;
    std::cout << " output indices: ";
    for (auto i : node.output_indices) {
      std::cout << i << ", ";
    }
    std::cout << std::endl;
    std::cout << " children: ";
    ...
    std::cout << std::endl;
    std::cout << " "
              << "first child: " << node.first_child << std::endl;
    std::cout << " "
              << "num children: " << node.num_children << std::endl;
    std::cout << " "
              << "next sibling: " << node.next_sibling << std::endl;
  }
  fflush(stdout);
}


void BM_test_path_1(nvbench::state& state)
{
  auto const paths = generate_paths_test_1();
  auto output      = spark_rapids_jni::generate_path_nodes(paths);
  print_nodes(output);
}

void BM_test_path_2(nvbench::state& state)
{
  auto const paths = generate_paths_test_2();
  auto output      = spark_rapids_jni::generate_path_nodes(paths);
  print_nodes(output);
}
#endif
...
// NVBENCH_BENCH(BM_test_path_1).set_name("test_path1");
// NVBENCH_BENCH(BM_test_path_2).set_name("test_path2");
