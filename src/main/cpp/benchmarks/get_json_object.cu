    ...
    if (method == 0) {
      ...
      for (int i = 0; i < 5; ++i) {
        if (paths[i].size() > 0) {
          ...
          std::size_t offset = 0;
          std::vector<std::vector<std::tuple<path_instruction_type, std::string, int32_t>>> segment;
          segment.reserve(threshold);
          while (offset < paths[i].size()) {
            auto const last = std::min(offset + threshold, paths[i].size());
            segment.resize(0);
            ...
            [[maybe_unused]] auto const output = spark_rapids_jni::get_json_object_multiple_paths(
              cudf::strings_column_view{chunk.tbl->get_column(i).view()}, segment, -1, -1);
          }
        }
      }
    }
