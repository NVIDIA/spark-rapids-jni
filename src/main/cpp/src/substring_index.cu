 ...
 */
        ...
        void compute_substring_indices(cudf::column_device_view const &d_column,
            ...
            thrust::for_each_n(
                    ...
                    strings_count,
                    [delim_itr, delimiter_count, start_char_pos, end_char_pos, d_column] __device__(cudf::size_type idx) {
    ...
    std::unique_ptr <cudf::column> substring_index(cudf::strings_column_view const &strings,
        ...
        auto start_chars_pos_vec = cudf::make_column_from_scalar(cudf::numeric_scalar<cudf::size_type>(0, true, stream),
                                                           strings_count,
                                                           stream,
                                                           rmm::mr::get_current_device_resource());
        auto stop_chars_pos_vec = cudf::make_column_from_scalar(cudf::numeric_scalar<cudf::size_type>(0, true, stream),
                                                          strings_count,
                                                          stream,
                                                          rmm::mr::get_current_device_resource());
