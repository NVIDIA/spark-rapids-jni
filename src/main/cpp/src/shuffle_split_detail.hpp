...
struct size_of_helper {
  template <typename T>
  constexpr std::enable_if_t<!cudf::is_fixed_width<T>() && !std::is_same_v<T, cudf::string_view>, size_t>
    operator()() const
  {
    return 0;
  }
  ...
  template <typename T>
  constexpr std::enable_if_t<cudf::is_fixed_width<T>(), size_t> __device__ operator()() const noexcept
  {
