...
JNIEXPORT jlong JNICALL Java_com_nvidia_spark_rapids_jni_Hash_sha224NullsPreserved(JNIEnv* env,
                                                                                   jclass,
                                                                                   jlong column_handle)
{
  ...
  {
    cudf::jni::auto_set_device(env);
    return cudf::jni::release_as_jlong(spark_rapids_jni::sha224_nulls_preserved(*reinterpret_cast<cudf::column_view const*>(column_handle)));
  }
  JNI_CATCH(env, 0);
}
}
