...
__device__ inline std::pair<cudf::size_type, cudf::bitmask_type> gpu_get_hash_mask(bloom_hash_type h, cudf::size_type bloom_filter_bits)
{
...
struct bloom_probe_functor {
  ...
  __device__ bool operator()(int64_t input)
  {
    ...
    for (auto idx = 1; idx <= num_hashes; idx++) {
      bloom_hash_type combined_hash = h1 + (idx * h2);
      auto const [word_index, mask] = gpu_get_hash_mask(combined_hash, bloom_filter_bits);
      if(!(bloom_filter[word_index] & mask)){
        return false;
      }
    }
    return true;
...
{
  rmm::device_uvector<cudf::bitmask_type> out(cudf::num_bitmask_words(bloom_filter_bits), stream, mr);
...
void bloom_filter_build(rmm::device_uvector<cudf::bitmask_type>& bloom_filter,
                        ...
                        rmm::cuda_stream_view stream)
{
  CUDF_EXPECTS(input.type() == cudf::data_type{cudf::type_id::INT64} && !input.nullable(), "bloom filter input expects a non-nullable column of int64s");

  constexpr int block_size = 256;
  auto grid        = cudf::detail::grid_1d{input.size(), block_size, 1};
  gpu_bloom_filter_build<<<grid.num_blocks, block_size, 0, stream.value()>>>(bloom_filter.data(),
                                                                             bloom_filter_bits,
                                                                             input,
                                                                             num_hashes);
}

rmm::device_uvector<cudf::bitmask_type> bloom_filter_merge(rmm::device_uvector<cudf::bitmask_type> const& a,
                                                           rmm::device_uvector<cudf::bitmask_type> const& b,
                                                           rmm::cuda_stream_view stream,
                                                           rmm::mr::device_memory_resource* mr)
{
  CUDF_EXPECTS(a.size() == b.size(), "bloom_filter_merge encountered mismatched input filter sizes");
  ...
  thrust::transform(rmm::exec_policy(stream),
                    ...
                    out.begin(),
                    [a = a.begin(), b = b.begin()] __device__(cudf::size_type i) {
                      return a[i] | b[i];
                    });
                                                 ...
                                                 rmm::mr::device_memory_resource* mr)
{
  ...
  auto out = cudf::make_fixed_width_column(cudf::data_type{cudf::type_id::BOOL8}, input.size(), cudf::mask_state::UNALLOCATED, stream, mr);  
  thrust::transform(rmm::exec_policy(stream),
                    input.begin<int64_t>(),
                    input.end<int64_t>(),
