...
struct average_example_groupby_udf : cudf::groupby_host_udf {
    ...
    rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr) const override
  {
    ...
    if (is_merge) {
      ...
      return spark_rapids_jni::group_merge_average_example(group_values, group_offsets, stream, mr);
    } else {
      // input is a int column
      return spark_rapids_jni::group_average_example(group_values, group_offsets, stream, mr);
    }
  }
...
struct average_example_reduction_udf : cudf::reduce_host_udf {
    ...
    rmm::device_async_resource_ref mr) const override
  {
    ...
    if (is_merge) {
      // input is a struct column: struct(sum, avg)
      return spark_rapids_jni::reduce_merge_average_example(input, stream, mr);
    } else {
      // intput is a int column
      return spark_rapids_jni::reduce_average_example(input, stream, mr);
    }
  }
