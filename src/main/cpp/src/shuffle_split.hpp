...
/**
 * @brief Buffer slice representing a portion of the single allocated buffer
 */
struct buffer_slice {
  uint8_t* data;  // pointer into the single allocation
  size_t size;    // size of this slice
  size_t offset;  // offset from the beginning of the single allocation
...
/**
 * @brief Single allocation result with buffer slices and native column_view handles for zero-copy column reconstruction
 */
struct single_allocation_buffers {
  rmm::device_buffer single_buffer;                    // THE single allocation
  std::vector<buffer_slice> buffer_slices;            // slices into the single buffer (needed for cudf::column_view creation)
  std::vector<std::unique_ptr<cudf::column_view>> column_views;  // non-owning column_view objects pointing to slices in single_buffer

  single_allocation_buffers(rmm::device_buffer&& buf,
                           std::vector<buffer_slice>&& slices)
    : single_buffer(std::move(buf)),
      buffer_slices(std::move(slices)),
      column_views{} {}
};

/**
 * @brief Reassembles a set of partitions generated by shuffle_split using single allocation approach.
 *
 ...
 * @param stream CUDA stream used for device memory operations and kernel launches.
 * @param mr Device memory resource used to allocate the returned column's device memory.
 * @return Single allocation buffers with slices for each column's data
 */
