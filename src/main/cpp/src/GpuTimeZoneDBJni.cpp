...
JNIEXPORT jlong JNICALL Java_com_nvidia_spark_rapids_jni_GpuTimeZoneDB_timeAddCS(
  JNIEnv* env, jclass, jlong input_handle, jlong duration, jlong transitions_handle, jint tz_index)
{
  JNI_NULL_CHECK(env, input_handle, "column is null", 0);
  JNI_NULL_CHECK(env, transitions_handle, "column is null", 0);
  try {
