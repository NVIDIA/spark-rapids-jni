                ...
                rmm::device_async_resource_ref mr)
{
  ...
  // Fill corresponding right indices with sentinel values (null markers)
                ...
                rmm::device_async_resource_ref mr)
{
  ...
  auto const num_left_unmatched = thrust::count(
    rmm::exec_policy_nosync(stream), left_has_match.begin(), left_has_match.end(), false);
  auto const num_right_unmatched = thrust::count(
  ...
  auto left_unmatched_iter = out_left_indices.begin() + offset;
  thrust::copy_if(rmm::exec_policy_nosync(stream),
                  thrust::counting_iterator<cudf::size_type>(0),
                  thrust::counting_iterator<cudf::size_type>(left_table_size),
                  left_has_match.begin(),
                  left_unmatched_iter,
                  cuda::std::logical_not<bool>());
  // Fill unmatched left rows with sentinel for right indices
  thrust::fill(rmm::exec_policy_nosync(stream),
               out_right_indices.begin() + offset,
               out_right_indices.begin() + offset + num_left_unmatched,
               std::numeric_limits<cudf::size_type>::min());

  offset += num_left_unmatched;

  // Add unmatched right rows
  auto right_unmatched_iter = out_right_indices.begin() + offset;
  thrust::copy_if(rmm::exec_policy_nosync(stream),
                  thrust::counting_iterator<cudf::size_type>(0),
                  thrust::counting_iterator<cudf::size_type>(right_table_size),
                  right_has_match.begin(),
                  right_unmatched_iter,
                  cuda::std::logical_not<bool>());
  // Fill unmatched right rows with sentinel for left indices
  thrust::fill(rmm::exec_policy_nosync(stream),
               out_left_indices.begin() + offset,
               out_left_indices.end(),
               std::numeric_limits<cudf::size_type>::min());
