  ...
  cudf::host_span<cudf::host_span<uint8_t const> const> inputs, CodecFunc&& codec_fn)
{
  ...
  std::vector<std::size_t> task_order(inputs.size());
  ...
  for (auto const idx : task_order) {
    auto task = [inp = inputs[idx], idx, codec_fn] { return std::pair{idx, codec_fn(inp, idx)}; };
                                      ...
                                      cudf::io::compression_type compression)
{
  ...
  auto codec_tasks =
    codec(inputs, [compression](cudf::host_span<uint8_t const> input, std::size_t) {
  ...
  std::vector<uint64_t> offsets(num_chunks + 1);
  std::vector<cudf::io::compression_type> buff_compressions(num_chunks);

  offsets.front() = 0;  // The first offset is always 0.
  ...
  // Write the data for each buffer.
  for (std::size_t idx = 0; idx < num_chunks; ++idx) {
...
std::vector<std::vector<uint8_t>> deserialize_cookie(cudf::host_span<uint8_t const> input)
{
  ...
  std::vector<cudf::host_span<uint8_t const>> input_buffers(num_chunks);
  std::vector<cudf::io::compression_type> compression_types(num_chunks);
  uint32_t check_sum = 0;

  for (std::size_t idx = 0; idx < num_chunks; ++idx) {
    ptr += copy(compression_types.data() + idx, ptr, sizeof(int32_t));

    // The offsets define the range of the serialized data, which also contains two extra integer
    // values for compression type and checksum.
    auto const buff_size = offsets[idx + 1] - offsets[idx] - 2 * sizeof(uint32_t);
    input_buffers[idx]   = cudf::host_span<uint8_t const>{ptr, buff_size};
    ptr += buff_size;

    ptr += copy(&check_sum, ptr, sizeof(uint32_t));  // unused for now
  }
  CUDF_EXPECTS(ptr == input.data() + input.size(),
               "Internal error: Cookie deserialization data size mismatch.");

  // Decompress the data into separate output buffers.
  auto codec_tasks = codec(
    input_buffers, [&compression_types](cudf::host_span<uint8_t const> input, std::size_t idx) {
