...
public class GpuTimeZoneDB {
   ...
   *
   * <p><b>Thread-safety:</b> instances are <i>not</i> thread-safe. A single
   * instance must not be shared across threads; each thread that needs to
   * perform conversions should obtain its own context via
   * {@link #buildOrcTimezoneContext(String, String)}. {@link #close()} is
   * idempotent against repeated calls on the same thread, but provides no
   * protection against concurrent close/use from multiple threads.
   */
