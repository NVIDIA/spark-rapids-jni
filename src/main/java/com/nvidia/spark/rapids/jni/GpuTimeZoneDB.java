 ...
 *    `cacheDatabaseImpl` and `shutdownImpl` are synchronized functions. This means only one can
 *    be running concurrently. `cacheDatabaseImpl` guarantees only load transitions cache once.
 *    The `shutdownImpl` will close the singleton host column vector generated by cacheDatabaseImpl
...
public class GpuTimeZoneDB {
   ...
   * Multiple concurrent running will be serial because of synchronization.
