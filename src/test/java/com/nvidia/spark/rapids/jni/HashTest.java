...
// IEEE 754 NaN values
  ...
  void testXXHash64Strings() {
    try (ColumnVector v0 = ColumnVector.fromStrings(
         ...
         ColumnVector expected = ColumnVector.fromBoxedLongs(-8582455328737087284L, 2221214721321197934L, 5798966295358745941L, -4834097201550955483L, -3782648123388245694L, Hash.SPARK_DEFAULT_XXHASH64_SEED)) {
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Ints() {
    try (ColumnVector v0 = ColumnVector.fromBoxedInts(0, 100, null, null, Integer.MIN_VALUE, null);
         ...
         ColumnVector expected = ColumnVector.fromBoxedLongs(1151812168208346021L, -7987742665087449293L, 8990748234399402673L, Hash.SPARK_DEFAULT_XXHASH64_SEED, 2073849959933241805L, 1508894993788531228L)) {
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Doubles() {
    try (ColumnVector v = ColumnVector.fromBoxedDoubles(
         ...
         ColumnVector expected = ColumnVector.fromBoxedLongs(-5252525462095825812L, Hash.SPARK_DEFAULT_XXHASH64_SEED, -7996023612001835843L, 5695175288042369293L, 6181148431538304986L, -4222314252576420879L, -3127944061524951246L, -3127944061524951246L, -3127944061524951246L, -3127944061524951246L, 5810986238603807492L, 5326262080505358431L)) {
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Timestamps() {
    ...
    try (ColumnVector v = ColumnVector.timestampMicroSecondsFromBoxedLongs(
        0L, null, 100L, -100L, 0x123456789abcdefL, null, -0x123456789abcdefL);
         ColumnVector result = Hash.xxhash64(new ColumnVector[]{v});
         ColumnVector expected = ColumnVector.fromBoxedLongs(-5252525462095825812L, Hash.SPARK_DEFAULT_XXHASH64_SEED, 8713583529807266080L, 5675770457807661948L, 1941233597257011502L, Hash.SPARK_DEFAULT_XXHASH64_SEED, -1318946533059658749L)) {
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Dates() {
    ...
    try (ColumnVector v = ColumnVector.timestampDaysFromBoxedInts(
        0, null, 100, -100, 0x12345678, null, -0x12345678);
         ColumnVector result = Hash.xxhash64(new ColumnVector[]{v});
         ColumnVector expected = ColumnVector.fromBoxedLongs(3614696996920510707L, Hash.SPARK_DEFAULT_XXHASH64_SEED, -7987742665087449293L, 8990748234399402673L, 6954428822481665164L, Hash.SPARK_DEFAULT_XXHASH64_SEED, -4294222333805341278L)) {
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Floats() {
    try (ColumnVector v = ColumnVector.fromBoxedFloats(
          ...
          Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY);
         ColumnVector result = Hash.xxhash64(new ColumnVector[]{v});
         ColumnVector expected = ColumnVector.fromBoxedLongs(3614696996920510707L, -8232251799677946044L, -6625719127870404449L, -6699704595004115126L, -1065250890878313112L, Hash.SPARK_DEFAULT_XXHASH64_SEED, 2692338816207849720L, 2692338816207849720L, 2692338816207849720L, 2692338816207849720L, -5940311692336719973L, -7580553461823983095L)){
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Bools() {
    try (ColumnVector v0 = ColumnVector.fromBoxedBooleans(null, true, false, true, null, false);
         ColumnVector v1 = ColumnVector.fromBoxedBooleans(null, true, false, null, false, true);
         ColumnVector result = Hash.xxhash64(new ColumnVector[]{v0, v1});
         ColumnVector expected = ColumnVector.fromBoxedLongs(Hash.SPARK_DEFAULT_XXHASH64_SEED, 9083826852238114423L, 1151812168208346021L, -6698625589789238999L, 3614696996920510707L, 7945966957015589024L)) {
      assertColumnsAreEqual(expected, result);
    }
  }
  ...
  void testXXHash64Mixed() {
    try (ColumnVector strings = ColumnVector.fromStrings(
         ...
         ColumnVector expected = ColumnVector.fromBoxedLongs(7451748878409563026L, 6024043102550151964L, 3380664624738534402L, 8444697026100086329L, -5888679192448042852L, Hash.SPARK_DEFAULT_XXHASH64_SEED)) {
      assertColumnsAreEqual(expected, result);
    }
  }
}
